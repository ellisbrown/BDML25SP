...
def main():
    """Main function for Tensor Parallel distributed training"""
    # Set up logging
    utils.setup_logging()
    ...
    # Create DeepSpeed engine with Tensor Parallelism config
    ...
    # Make sure tensor parallelism is enabled in the config
    if "tensor_parallel" not in ds_config:
        logging.warning("Tensor parallelism not specified in config. Adding default configuration.")
        ds_config["tensor_parallel"] = {
            "enabled": True,
            "tp_size": world_size,
            "tp_mode": "LINEAR"  # alternatives: CUBE, SHARD
        }

    # Initialize DeepSpeed with tensor parallelism
